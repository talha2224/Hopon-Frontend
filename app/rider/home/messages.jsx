...
const Messages = () => {
    ...
    const handleSendMessage = () => {
        if (message.trim()) {
            ...
            setMessages((prev) => [...prev, { ...newMessage, sender: 'rider' }]);
            setMessage('');
        }
    };
    ...
    return (
        <View style={isDarkTheme ? style.containerDark : style.container}>
            ...
            <View style={{ paddingHorizontal: 20, paddingVertical: 40, flexDirection: 'row', alignItems: 'center' }}>
                <AntDesign onPress={() => router.push('/rider/home/msg')} name="arrowleft" size={24} color={isDarkTheme ? 'white' : 'black'} />
            ...
            <FlatList
                ...
                renderItem={({ item }) => {
                    // Safeguard to ensure `item.message` is a string
                    const displayMessage = typeof item.message === 'string' ? item.message : JSON.stringify(item.message);

                    return (
                        <View
                            style={{
                                alignSelf: item?.senderRole === 'rider' ? 'flex-end' : 'flex-start',
                                backgroundColor: item?.senderRole === 'rider' ? '#2666CF' : '#e0e0e0',
                                padding: 10,
                                borderRadius: 10,
                                marginVertical: 5,
                                marginHorizontal: 10,
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flexDirection: 'row', alignItems: 'center', padding: 10, backgroundColor: isDarkTheme ? '#333' : '#fff' }}>
                <TextInput value={message} onChangeText={setMessage} placeholder="Type your message..." placeholderTextColor={isDarkTheme ? '#888' : '#aaa'} style={{ flex: 1, height: 40, borderColor: isDarkTheme ? '#555' : '#ccc', borderWidth: 1, borderRadius: 20, paddingHorizontal: 10, color: isDarkTheme ? 'white' : 'black', }} />
