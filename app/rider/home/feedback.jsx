...
const Feedback = () => {
    ...
    const [selectedTip, setSelectedTip] = useState(null); // State to track selected tip
    ...
    return (
        <View style={{ backgroundColor: isDarkTheme ? 'black' : '#fcfdff', flex: 1, alignItems: 'center' }}>
            ...
            <View style={{ backgroundColor: isDarkTheme ? '#2E2D2D' : 'black', width: '100%', paddingBottom: 10 }}>
                <Text style={{ textAlign: 'center', color: 'white', marginBottom: 10, fontWeight: '800', fontSize: 16, marginTop: 30 }}>
                    Thanks for riding, {data?.rider?.first_name}
                </Text>
            ...
            <View style={{ marginTop: 10, backgroundColor: isDarkTheme ? '#292929' : '#FAFAFA', padding: 10, width: '95%', margin: 10, borderRadius: isDarkTheme && 10 }}>
                <View style={{ alignItems: 'center', flexDirection: 'row', marginHorizontal: 20 }}>
                    ...
                    <View style={{ marginLeft: 20 }}>
                        ...
                        <Text style={{ color: '#A0A1A3', marginTop: 5 }}>
                            {data?.pickUpAddress?.split(' ').slice(0, 5).join(' ')}
                        </Text>
                        <Text style={{ marginTop: 10, color: isDarkTheme && 'white' }}>Drop Off</Text>
                        <Text style={{ color: '#A0A1A3', marginTop: 5 }}>
                            {data?.dropoffAddress?.split(' ').slice(0, 5).join(' ')}
                        </Text>
                    </View>
                </View>
            ...
            <Text style={{ textAlign: 'center', color: isDarkTheme ? 'white' : '#727272', marginVertical: 10 }}>
                Add a tip for {data?.driver?.first_name}
            </Text>

            <View style={{ marginTop: 10, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', padding: 10, width: '95%', margin: 10 }}>
                {['$5', '$10', '$15', '$20'].map((tip) => (
                    <TouchableOpacity key={tip} onPress={() => handleTipSelect(tip)} style={{backgroundColor: selectedTip === tip ? '#2666cf' : isDarkTheme ? '#292929' : '#F1F1F1',padding: 10,marginRight: 10,borderRadius: 10,justifyContent: 'center',alignItems: 'center',width: 70,}}>
                        <Text style={{ color: isDarkTheme && 'white' }}>{tip}</Text>
                    </TouchableOpacity>
                ))}
            </View>
            ...
            <View style={{ backgroundColor: '#2666cf', width: '95%', height: 40, justifyContent: 'center', alignItems: 'center', marginVertical: 13, borderRadius: 8 }}>
                <Text onPress={() => router.push('/rider/home')} style={{ color: '#fff' }}>
                    Done
                </Text>
            </View>
        </View>
